...
def import_model_constants():
	...
	for line in f.readlines():
		...
		if len(macro) > 1:
			...
			val = val.replace("f", "")
			# this awful eval is needed in order to get the right results out of macro definitions
			tmp = eval(val)
			# print(val, tmp)
			globals()[key] = tmp
	f.close()
...
class Logger(object):
	...
	def log(msg, level=None, additional=None):
		...
		if additional != None and Logger.debug_level == Logger.DEBUG:
			print("%s: %s (additional info: '%s')" % (Logger.messages[level], msg, additional))
		else:
...
class Nucleotide(Printable):
	...
	def add_H_interaction (self,nucleotide):
		#if abs(self.index-nucleotide) != 1:
			#print("adding bond between " + str(self.index) + " and " + str(nucleotide))
...
class Strand(Printable):
	...
	def append (self, other):
		...
		if np.sqrt(np.dot (dr, dr)) > (0.7525 + 0.25):
			print("WARNING: Strand.append(): strands seem too far apart. Assuming you know what you are doing.")
...
class System(object):
	...
	def read_H_bonds(self, inputpipe):
		for line in inputpipe:
			if( len(line) and line[0] == '#' and 't = ' in line) : 
				self._time = int(line[ line.index('t = ')+4 : ])
	...
	def add_H_interaction(self,nuclA,nuclB,interaction):
		...
		if(interaction < H_CUTOFF):
			...
			# [MRS] : shifted these into the "if" statement so that nucleotide objects and strand objects agree
			self._nucleotides[nuclA].add_H_interaction(nuclB)
			self._nucleotides[nuclB].add_H_interaction(nuclA)

	def get_H_interactions_nucleotides(self):
		retset = []
		for i in range(len(self._nucleotides)):
			interactions = self._nucleotides[i].get_H_interactions()
			for j in interactions:
				if i < j and ( j-i != 1 or self._nucleotide_to_strand[i] != self._nucleotide_to_strand[j] ):
					retset.append( [i,j] )
		return retset

	def get_H_interactions(self):
		retset = [[int(0) for x in range(len(self._strands))] for z in range(len(self._strands))]
		for i in range(len(self._strands)):
			interactions = self._strands[i].get_H_interactions()
			for j in interactions.keys():
				retset[i][j] = interactions[j]
		return retset
	
	def get_num_strands(self):
		return len(self._strands)
	
	def get_num_nucleotides(self):
		return len(self._nucleotides)
